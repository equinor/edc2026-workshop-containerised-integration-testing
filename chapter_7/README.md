...
**`test_check_stock`** — the first test written against the Train Logistics&trade; API. Make a `POST` request to `/logistics/check-stock` with a `train_code` and `product` and assert the response contains the correct stock level.
